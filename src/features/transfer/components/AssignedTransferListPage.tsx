...
export function AssignedTransferListPage(): ReactElement {
  ...
  const getCellText = (row: TransferHeader, key: TransferAssignedColumnKey): string | undefined => {
    switch (key) {
  ...
  return (
    <>
      <OpsListPageShell
        ...
        title={t('transfer.assignedList.title')}
        description={t('transfer.assignedList.subtitle', { defaultValue: t('transfer.list.subtitle') })}
      >
        ...
        <PagedDataGrid<TransferHeader, TransferAssignedColumnKey>
          variant="ops"
          columns={columns}
          visibleColumnKeys={orderedVisibleColumns.filter((key) => key !== 'actions') as TransferAssignedColumnKey[]}
          ...
          } as Record<Exclude<TransferAssignedColumnKey, 'actions'>, React.ReactNode>)[columnKey as Exclude<TransferAssignedColumnKey, 'actions'>] ?? null}
