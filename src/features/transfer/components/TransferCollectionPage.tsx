...
export function TransferCollectionPage(): ReactElement {
  ...
  const { t } = useTranslation(['transfer', 'common', 'transfer-chain']);
  ...
  return (
    <div className="wms-ops-form wms-ops-collection wms-ops-transfer-collection h-[calc(100vh-10rem)] min-h-[32rem]">
      <div className="wms-ops-collection__toolbar">
        <div className="wms-ops-collection__toolbar-group">
          <OpsActionButton type="button" variant="secondary" onClick={() => navigate('/transfer/assigned')}>
        ...
        <OpsActionButton
          type="button"
          variant="secondary"
          onClick={() => navigate(`/transfer/collected/${headerId}`)}
        >
      ...
      <div className="wms-ops-collection__scan">
        <div className="wms-ops-collection__scan-row">
          <OpsFieldShell
            ...
            title={!barcodeInput.trim() ? t('transfer.collection.barcodePlaceholder') : undefined}
          >
            ...
            <button
              ...
              aria-label={t('transfer.collection.scanBarcode')}
            >
        ...
        {selectedStock ? (
          <div className="wms-ops-collection__panel">
            ...
            <div className="wms-ops-collection__collect-row">
              ...
              <div>
                <span className="wms-ops-collection__field-label">{t('transfer.collection.quantity')}</span>
                <OpsFieldShell>
                  <Input
                    type="number"
                    min="1"
      ...
      <div className="wms-ops-collection__body custom-scrollbar">
          ...
          <div ref={collectedSectionRef} className="mb-4">
            <div className="wms-ops-collection__collected-banner">
              ...
              <span className="wms-ops-collection__collected-total">
                {t('transfer.collection.total')}: {totalCollectedQuantity}
              </span>
            </div>
                ...
                <div className="wms-ops-collection__stock-head">
                  <div className="min-w-0">
                    ...
                    {item.importLine.yapKod ? (
                      <p className="wms-ops-collection__stock-meta">
                        {t('transfer.collection.yapKod')}: {item.importLine.yapKod}
                      </p>
                    ) : null}
                  </div>
                ...
                {item.routes.map((route) => (
                  <div key={route.id} className="wms-ops-collection__route-item">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      ...
                      <span className="wms-ops-collection__unit-badge">
                        {t('transfer.collection.quantity')}: {route.quantity}
                      </span>
                    </div>
                    {(route.serialNo || route.sourceCellCode) ? (
                      <div className="wms-ops-collection__route-meta">
                        {route.serialNo ? <span>{t('transfer.collection.serialNo')}: {route.serialNo}</span> : null}
        ...
        <div className="wms-ops-collection__section-title">
          <span>{t('transfer.collection.title')}</span>
          {orderLinesWithCollected.length > 0 ? (
            <span>{orderLinesWithCollected.length} {t('transfer.collection.itemsCollected')}</span>
          ) : null}
        </div>
        ...
        {isLoadingOrderLines ? (
                ...
                >
                  ...
                  <div className="wms-ops-collection__metrics">
                    <div className="wms-ops-collection__metric">
                      <div className="wms-ops-collection__metric-label">{t('transfer.collection.total')}</div>
                      <div className="wms-ops-collection__metric-value">{line.quantity}</div>
                    </div>
                    <div className="wms-ops-collection__metric">
                      <div className="wms-ops-collection__metric-label">{t('transfer.collection.collected')}</div>
                      <div className="wms-ops-collection__metric-value wms-ops-collection__metric-value--ok">
                        {line.collectedQuantity}
                      </div>
                    </div>
                    <div className="wms-ops-collection__metric">
                      <div className="wms-ops-collection__metric-label">{t('transfer.collection.remaining')}</div>
                      <div
                        className={cn(
          ...
          <div className="wms-ops-collection__empty">{t('transfer.collection.noOrderLines')}</div>
        )}
      </div>

      <div className="wms-ops-transfer-collection__chain">
      ...
      >
        <DialogContent className="wms-ops-form wms-ops-detail-dialog max-w-[95vw] gap-0 overflow-hidden border-0 p-0 shadow-none sm:max-w-lg">
          <DialogHeader className="wms-ops-detail-dialog__header border-b px-6 py-4">
            <DialogTitle className="wms-ops-detail-dialog__title">{t('transfer.collection.scanBarcode')}</DialogTitle>
