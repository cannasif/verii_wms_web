...
interface TransferItemRowProps {
  ...
  selectedItem?: SelectedTransferOrderItem;
...
export function TransferItemRow({
  ...
  onToggleItem,
  onRemoveItem,
}: TransferItemRowProps): ReactElement {
  ...
  const handleDetailChange = (
    field: keyof SelectedTransferOrderItem,
    val: string | number
  ): void => {
    if (!selectedItem) return;
    const itemId = item.id || '';
    onUpdateItem(itemId, { [field]: val } as Partial<SelectedTransferOrderItem>);
  };
