...
export function ShipmentApprovalPage(): ReactElement {
  ...
  const pageKey = 'shipment-approval-list';
  ...
  const columns = useMemo<PagedDataGridColumn<ColumnKey>[]>(() => [
    {
      key: 'id',
      label: t('shipment.approval.id'),
      headClassName: 'wms-ops-table-id-col wms-ops-table-center-col',
      cellClassName: 'wms-ops-table-id-col wms-ops-table-center-col',
    },
    { key: 'documentNo', label: t('shipment.approval.documentNo'), headClassName: 'wms-ops-table-center-col', cellClassName: 'wms-ops-table-center-col' },
    { key: 'documentDate', label: t('shipment.approval.documentDate'), headClassName: 'wms-ops-table-center-col', cellClassName: 'wms-ops-table-center-col' },
    { key: 'customerCode', label: t('shipment.approval.customerCode'), headClassName: 'wms-ops-table-center-col', cellClassName: 'wms-ops-table-center-col' },
    { key: 'customerName', label: t('shipment.approval.customerName'), headClassName: 'wms-ops-table-center-col', cellClassName: 'wms-ops-table-center-col' },
    { key: 'sourceWarehouse', label: t('shipment.approval.sourceWarehouse'), headClassName: 'wms-ops-table-center-col', cellClassName: 'wms-ops-table-center-col' },
    { key: 'targetWarehouse', label: t('shipment.approval.targetWarehouse'), headClassName: 'wms-ops-table-center-col', cellClassName: 'wms-ops-table-center-col' },
    { key: 'completionDate', label: t('shipment.approval.completionDate'), headClassName: 'wms-ops-table-center-col', cellClassName: 'wms-ops-table-center-col' },
  ...
  const getCellText = (row: ShipmentHeader, key: ColumnKey): string | undefined => {
    switch (key) {
  ...
  const handleApproval = async (id: number, approved: boolean): Promise<void> => {
    ...
    try {
      ...
      toast.success(approved ? t('shipment.approval.approveSuccess') : t('shipment.approval.rejectSuccess'));
    } catch (err) {
      toast.error(err instanceof Error ? err.message : approved ? t('shipment.approval.approveError') : t('shipment.approval.rejectError'));
    }
  };
  ...
  return (
    <>
      <OpsListPageShell
        ...
        title={t('shipment.approval.title')}
        description={t('shipment.approval.subtitle', { defaultValue: t('shipment.list.subtitle') })}
      >
        <PagedDataGrid<ShipmentHeader, ColumnKey>
          ...
          renderActionsCell={(row) => (
            <div className="wms-ops-row-actions">
              <Button
                ...
                aria-label={t('shipment.approval.viewDetails')}
                title={t('shipment.approval.viewDetails')}
              ...
              <Button
                ...
                aria-label={t('shipment.approval.approve')}
                title={t('shipment.approval.approve')}
              ...
              <Button
                ...
                aria-label={t('shipment.approval.reject')}
                title={t('shipment.approval.reject')}
                disabled={!permission.canApprove || approveMutation.isPending}
          ...
          actionBar={{
            ...
            search: { ...pagedGrid.searchConfig, placeholder: t('shipment.approval.searchPlaceholder') },
            leftSlot: (
              <VoiceSearchButton
      ...
      {selectedHeaderId && (
        <ShipmentDetailDialog headerId={selectedHeaderId} isOpen onClose={() => setSelectedHeaderId(null)} />
      )}
    </>
  );
}
