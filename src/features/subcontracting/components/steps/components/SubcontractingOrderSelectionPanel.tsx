...
import type { SubcontractingOrder } from '../../../types/subcontracting';

interface SubcontractingOrderSelectionPanelProps {
  orders: SubcontractingOrder[];
  selectedOrderId: string | null;
  onSelectOrder: (orderId: string) => void;
  isLoading: boolean;
}

export function SubcontractingOrderSelectionPanel({
  orders,
  selectedOrderId,
  onSelectOrder,
  isLoading,
}: SubcontractingOrderSelectionPanelProps): ReactElement {
            ...
            >
              <div className="flex items-start justify-between gap-1.5 mb-1.5">
                ...
                >
                  {order.remainingForImport > 0
                    ? t('subcontracting.step2.pending', 'Beklemede')
                    : t('subcontracting.step2.completed', 'Tamamlandı')}
                </Badge>
              </div>
              <div className="flex items-center justify-between text-[10px] text-muted-foreground">
                ...
                <span className="font-medium">
                  {t('subcontracting.step2.remaining', 'Kalan')}: {order.remainingForImport.toFixed(2)}
                </span>
              </div>
            </div>
          ))
