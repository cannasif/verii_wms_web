...
export function SitCollectionPage(): ReactElement {
  ...
  const { data: barcodeData, isLoading: isSearching, error: barcodeError, isError: isBarcodeError } = useSitStokBarcode(searchBarcode, enableSearch);
  const barcodeDefinitionQuery = useQuery({
    queryKey: ['barcode-definition', 'subcontracting-issue-assigned'],
    queryFn: ({ signal }) => barcodeApi.getDefinition('subcontracting-issue-assigned', { signal }),
  });
  ...
  useEffect(() => {
    ...
    const feedback = extractBarcodeFeedback(barcodeError, t('subcontracting.sit.collection.stockNotFound'));
  ...
  const handleCollect = (): void => {
    ...
    addBarcodeMutation.mutate(
      ...
      },
      {
        ...
        onError: (error: Error) => {
          const feedback = extractBarcodeFeedback(error, t('subcontracting.sit.collection.collectError'));
          toast.error(feedback.message);
        },
      }
    );
    ...
    <div className="wms-ops-form wms-ops-collection h-[calc(100vh-10rem)] min-h-[32rem]">
      <div className="wms-ops-collection__toolbar">
        <div className="wms-ops-collection__toolbar-group">
          <OpsActionButton type="button" variant="secondary" onClick={() => navigate('/subcontracting/issue/assigned')}>
      ...
      <div className="wms-ops-collection__scan">
        <div className="wms-ops-collection__scan-row">
          <OpsFieldShell className="wms-ops-collection__barcode-field" title={!barcodeInput.trim() ? t('subcontracting.sit.collection.barcodePlaceholder') : undefined}>
            <Barcode className="wms-ops-collection__barcode-icon" aria-hidden />
            <button type="button" className="wms-ops-collection__camera-btn" disabled={!permission.canUpdate} onClick={handleOpenCamera} aria-label={t('subcontracting.sit.collection.scanBarcode')}>
              <Camera className="size-3.5" aria-hidden />
            </button>
            <Input placeholder={t('subcontracting.sit.collection.barcodePlaceholder')} value={barcodeInput} onChange={(e) => setBarcodeInput(e.target.value)} onKeyPress={handleKeyPress} disabled={!permission.canUpdate} className={cn(OPS_FIELD_CLASS, 'wms-ops-collection__barcode-input')} />
          </OpsFieldShell>
        ...
        {selectedStock ? (
          <div className="wms-ops-collection__panel">
            ...
            <div className="wms-ops-collection__collect-row">
              ...
              <div>
                <span className="wms-ops-collection__field-label">{t('subcontracting.sit.collection.quantity')}</span>
                <OpsFieldShell>
      ...
      <div className="wms-ops-collection__body custom-scrollbar">
        {isLoadingOrderLines ? (
                ...
                <div key={line.id} className={cn('wms-ops-collection__line-card', isDone && 'wms-ops-collection__line-card--done')}>
                  <div className="wms-ops-collection__line-head">
                    <div className="min-w-0 flex-1">
                      ...
                      {line.yapKod ? <p className="wms-ops-collection__stock-meta">{t('subcontracting.sit.collection.yapKod')}: {line.yapKod}{line.yapAcik ? ` · ${line.yapAcik}` : ''}</p> : null}
                    </div>
                    {isDone ? <CheckCircle2 className="size-4 shrink-0 text-emerald-500" aria-hidden /> : null}
                  </div>
                  <div className="wms-ops-collection__metrics">
                    <div className="wms-ops-collection__metric"><div className="wms-ops-collection__metric-label">{t('subcontracting.sit.collection.total')}</div><div className="wms-ops-collection__metric-value">{line.quantity}</div></div>
                    <div className="wms-ops-collection__metric"><div className="wms-ops-collection__metric-label">{t('subcontracting.sit.collection.collected')}</div><div className="wms-ops-collection__metric-value wms-ops-collection__metric-value--ok">{line.collectedQuantity}</div></div>
                    <div className="wms-ops-collection__metric"><div className="wms-ops-collection__metric-label">{t('subcontracting.sit.collection.remaining')}</div><div className={cn('wms-ops-collection__metric-value', isDone ? 'wms-ops-collection__metric-value--ok' : 'wms-ops-collection__metric-value--warn')}>{line.remainingQuantity}</div></div>
                  </div>
          ...
          <div className="wms-ops-collection__empty">{t('subcontracting.sit.collection.noOrderLines')}</div>
        )}
      </div>

      <div className="wms-ops-collection__footer">
        <OpsActionButton type="button" variant="primary" onClick={handleComplete} disabled={!permission.canUpdate || completeSitMutation.isPending}>
          {completeSitMutation.isPending ? <Loader2 className="size-3.5 animate-spin" aria-hidden /> : <Check className="size-3.5" aria-hidden />}
      ...
      <Dialog open={isCameraOpen} onOpenChange={(open) => { if (!open) handleCloseCamera(); }}>
        <DialogContent className="wms-ops-form wms-ops-detail-dialog max-w-[95vw] gap-0 overflow-hidden border-0 p-0 shadow-none sm:max-w-lg">
          <DialogHeader className="wms-ops-detail-dialog__header border-b px-6 py-4">
            <DialogTitle className="wms-ops-detail-dialog__title">{t('subcontracting.sit.collection.scanBarcode')}</DialogTitle>
            <DialogDescription className="wms-ops-detail-dialog__description">{t('subcontracting.sit.collection.scanBarcodeDescription')}</DialogDescription>
          </DialogHeader>
          <div className="relative w-full" style={{ minHeight: '300px' }}>
            <div id="barcode-scanner-sit" ref={scannerContainerRef} className="w-full" style={{ minHeight: '300px' }} />
          </div>
        </DialogContent>
      </Dialog>
    </div>
