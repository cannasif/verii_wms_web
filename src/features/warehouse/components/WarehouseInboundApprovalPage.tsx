...
export function WarehouseInboundApprovalPage(): ReactElement {
  ...
  const columns = useMemo<ColumnDef[]>(
    () => [
      ...
      { key: 'actions', label: t('warehouse.inbound.approval.actions') },
    ],
    [t]
  );
  ...
  const handleApprove = async (id: number): Promise<void> => {
    try {
      ...
      toast.success(t('warehouse.inbound.approval.approveSuccess'));
    } catch (error) {
      toast.error(
        error instanceof Error
          ? error.message
          : t('warehouse.inbound.approval.approveError')
      );
    }
  };
  ...
  const handleReject = async (id: number): Promise<void> => {
    try {
      ...
      toast.success(t('warehouse.inbound.approval.rejectSuccess'));
    } catch (error) {
      toast.error(
        error instanceof Error
          ? error.message
          : t('warehouse.inbound.approval.rejectError')
      );
    }
  };
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <p className="text-destructive">
          {t('warehouse.inbound.approval.error')}
        </p>
      </div>
    );
  }
          ...
          <div className="crm-toolbar flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="relative flex items-center w-full md:w-auto">
                ...
                <Input
                  placeholder={t('warehouse.inbound.approval.searchPlaceholder')}
                  value={searchTerm}
                  onChange={(e) => {
              ...
              <TableHeader>
                <TableRow>
                  {orderedVisibleColumns.map((key) => {
                    if (key === 'id') return <TableHead key={key}>{t('warehouse.inbound.approval.id')}</TableHead>;
                    if (key === 'documentNo') return <TableHead key={key}>{t('warehouse.inbound.approval.documentNo')}</TableHead>;
                    if (key === 'documentDate') return <TableHead key={key}>{t('warehouse.inbound.approval.documentDate')}</TableHead>;
                    if (key === 'customerCode') return <TableHead key={key}>{t('warehouse.inbound.approval.customerCode')}</TableHead>;
                    if (key === 'customerName') return <TableHead key={key}>{t('warehouse.inbound.approval.customerName')}</TableHead>;
                    if (key === 'targetWarehouse') return <TableHead key={key}>{t('warehouse.inbound.approval.targetWarehouse')}</TableHead>;
                    if (key === 'completionDate') return <TableHead key={key}>{t('warehouse.inbound.approval.completionDate')}</TableHead>;
                    if (key === 'actions') return <TableHead key={key}>{t('warehouse.inbound.approval.actions')}</TableHead>;
                    return null;
                  })}
                </TableRow>
              </TableHeader>
                  ...
                  <TableRow>
                    <TableCell colSpan={Math.max(orderedVisibleColumns.length, 1)} className="text-center py-8">
                      <p className="text-muted-foreground">
                        {t('warehouse.inbound.approval.noData')}
                      </p>
                    </TableCell>
                  </TableRow>
                )}
                ...
                <Card key={item.id} className="border">
                  <CardContent className="p-4 space-y-3">
                    <div className="flex items-start justify-between">
                      <div>
                        <p className="text-sm font-medium text-muted-foreground">
                          {t('warehouse.inbound.approval.id')}
                        </p>
                    ...
                    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                      <div>
                        <p className="text-sm font-medium text-muted-foreground">
                          {t('warehouse.inbound.approval.documentNo')}
                        </p>
                        <p className="text-base">{item.documentNo || '-'}</p>
                      </div>
                      <div>
                        <p className="text-sm font-medium text-muted-foreground">
                          {t('warehouse.inbound.approval.documentDate')}
                        </p>
                        <p className="text-base">{formatDate(item.documentDate)}</p>
                      </div>
                      <div>
                        <p className="text-sm font-medium text-muted-foreground">
                          {t('warehouse.inbound.approval.customerCode')}
                        </p>
                        <p className="text-base">{item.customerCode || '-'}</p>
                      </div>
                      <div>
                        <p className="text-sm font-medium text-muted-foreground">
                          {t('warehouse.inbound.approval.customerName')}
                        </p>
                        <p className="text-base">{item.customerName || '-'}</p>
                      </div>
                      <div>
                        <p className="text-sm font-medium text-muted-foreground">
                          {t('warehouse.inbound.approval.targetWarehouse')}
                        </p>
                        <p className="text-base">{item.targetWarehouse || '-'}</p>
                      </div>
                      <div>
                        <p className="text-sm font-medium text-muted-foreground">
                          {t('warehouse.inbound.approval.completionDate')}
                        </p>
                    ...
                    <div className="pt-2 space-y-2">
                      ...
                      >
                        ...
                        {t('warehouse.inbound.approval.viewDetails')}
                      </Button>
                      <div className="flex gap-2">
                        ...
                        >
                          ...
                          {t('warehouse.inbound.approval.approve')}
                        </Button>
                        <Button
                          variant="destructive"
                          size="sm"
                        ...
                        >
                          ...
                          {t('warehouse.inbound.approval.reject')}
                        </Button>
                      </div>
                    </div>
                  </CardContent>
