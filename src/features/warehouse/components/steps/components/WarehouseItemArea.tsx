...
export function WarehouseItemArea({
  ...
  if (!siparisNo) {
    return isOps ? (
      <div className="wms-ops-panel-empty wms-ops-panel-empty--detail">
        ...
        <p className="wms-ops-panel-empty__title">{t('warehouse.step2.noOrderSelected')}</p>
        <p className="wms-ops-panel-empty__hint">{t('warehouse.step2.selectOrderPrompt')}</p>
      </div>
    ) : (
      <Card>
        <CardContent className="flex flex-col items-center justify-center py-12">
          <div className="space-y-4 text-center">
            ...
            <div className="space-y-2">
              <CardTitle>{t('warehouse.step2.noOrderSelected')}</CardTitle>
              <CardDescription>{t('warehouse.step2.selectOrderPrompt')}</CardDescription>
            </div>
          </div>
        </CardContent>
      </Card>
  ...
  if (!mappedOrderItems || mappedOrderItems.length === 0) {
    return isOps ? (
      <div className="wms-ops-panel-empty">
        <PackageOpen className="size-8" aria-hidden />
        <p>{t('warehouse.step2.noItemsFound')}</p>
      </div>
    ) : (
      <Card>
        <CardContent className="flex flex-col items-center justify-center py-12">
          <PackageOpen className="mb-4 h-12 w-12 text-muted-foreground opacity-50" />
          <p className="text-sm text-muted-foreground">{t('warehouse.step2.noItemsFound')}</p>
        </CardContent>
      </Card>
    );
  }
        ...
        <div className="flex items-center justify-between gap-2">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              {t('warehouse.step2.itemsCount', { count: totalItems })}
            </p>
          </div>
          <div className="text-right">
            <p className="text-xs text-muted-foreground">{t('warehouse.step2.started')}</p>
