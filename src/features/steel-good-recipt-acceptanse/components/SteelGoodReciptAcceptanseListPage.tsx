...
import { OpsInput, OpsListPageShell, OpsServiceEyebrow } from '@/components/shared';
      ...
      <div className="space-y-4">
        <div className="flex flex-wrap gap-3">
          <OpsInput
            ...
            className="max-w-md"
          />
          <OpsActionButton type="button" variant="secondary" onClick={() => setSearch(searchInput.trim())}>
            {t('common.search')}
          </OpsActionButton>
        ...
        <div className="wms-ops-table-wrap overflow-x-auto rounded-xl border">
          <table className="wms-ops-table min-w-[2100px] text-sm">
            <thead>
              <tr>
            ...
            <tbody>
              {rows.map((row) => (
                <tr key={row.id}>
                  ...
                  <td className="px-3 py-2 font-medium font-mono text-xs">{row.dCode}</td>
