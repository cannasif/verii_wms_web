...
const OPS_SELECT_TRIGGER = 'wms-ops-list-field-trigger h-8 min-w-0 w-full text-xs';
const OPS_SELECT_CONTENT = 'wms-ops-list-select-content rounded-none backdrop-blur-none';
...
export function AdvancedFilter({
  ...
  const renderToolbarButtons = (): ReactElement => (
    <div className={cn(isOps ? 'wms-ops-advanced-filter__toolbar' : 'flex flex-wrap gap-2')}>
      {isOps ? (
        <>
          <OpsActionButton type="button" variant="secondary" onClick={addRow}>
            <Plus className="size-3.5" aria-hidden />
            {getLabel('add')}
          </OpsActionButton>
          <OpsActionButton type="button" variant="secondary" onClick={onClear}>
            {getLabel('clear')}
          </OpsActionButton>
          <OpsActionButton type="button" variant="primary" onClick={onSearch}>
  ...
  return (
    <div
      className={cn(
        isOps
          ? 'wms-ops-advanced-filter'
          : embedded
            ? 'p-4 space-y-4'
            : 'rounded-xl border border-slate-200 dark:border-white/10 bg-white/50 dark:bg-card/50 p-4 space-y-4',
      )}
    >
      {!embedded && (
      ...
      {embedded && !isOps ? renderToolbarButtons() : null}
      {embedded && isOps ? renderToolbarButtons() : null}
      ...
      {appliedFilterCount > 0 && (
        <div
          className={cn(
            isOps
              ? 'wms-ops-advanced-filter__active'
              : 'rounded-md border border-emerald-200/70 bg-emerald-50 px-3 py-2 text-xs font-medium text-emerald-700 dark:border-emerald-500/30 dark:bg-emerald-500/10 dark:text-emerald-300',
          )}
        >
      ...
      {onFilterLogicChange && (
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
          <span
            className={cn(
              isOps
                ? 'wms-ops-advanced-filter__logic-label'
          ...
          <Select value={filterLogic} onValueChange={(value: 'and' | 'or') => onFilterLogicChange(value)}>
            <SelectTrigger className={cn(isOps ? OPS_SELECT_TRIGGER : 'w-full sm:w-[140px]')}>
              <SelectValue placeholder={getLabel('logic')} />
            </SelectTrigger>
            <SelectContent className={isOps ? OPS_SELECT_CONTENT : undefined}>
          ...
          {draftRows.map((row) => {
            ...
            return (
              <div
                key={row.id}
                className={cn(
                  isOps ? 'wms-ops-advanced-filter__row' : 'flex flex-wrap items-center gap-2',
                )}
              >
                <Select value={row.column} onValueChange={(value) => updateRow(row.id, { column: value })}>
                  <SelectTrigger className={cn(isOps ? OPS_SELECT_TRIGGER : 'w-full sm:w-[170px]')}>
                ...
                {config?.type === 'boolean' ? (
                  ...
                  <Input
                    ...
                    className={cn(isOps ? `${OPS_FIELD_CLASS} h-8 min-w-0 w-full text-xs` : 'w-full sm:w-[170px]')}
                  />
                )}
                {isOps ? (
                  <button
